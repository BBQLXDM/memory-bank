...
class TranscriptSegmentChunkerTest {
    ...
    void normalizedWhitespaceMatchReturnsOriginalBoundaryWithoutApproximateFlag() {
        ...
        assertThat(segments.get(0).metadata()).doesNotContainKey("boundaryApproximate");
        assertThat((CharBoundary) segments.get(0).boundary())
    ...
    void approximateFallbackMarksBoundaryAndStaysMonotonic() {
        ...
        assertThat(segments).hasSize(1);
        assertThat(segments.get(0).metadata()).containsEntry("boundaryApproximate", true);
        assertThat(((CharBoundary) segments.get(0).boundary()).startChar()).isZero();
    }
    ...
    void oversizedTranscriptChildrenReceiveProportionalTimeBounds() {
        ...
        assertThat(segments.get(0).metadata()).containsEntry("startTime", Duration.ZERO);
        assertThat(segments.get(0).metadata().get("endTime")).isNotEqualTo(Duration.ofMinutes(1));
        assertThat(segments.get(segments.size() - 1).metadata())
                .containsEntry("endTime", Duration.ofMinutes(1));
    }
