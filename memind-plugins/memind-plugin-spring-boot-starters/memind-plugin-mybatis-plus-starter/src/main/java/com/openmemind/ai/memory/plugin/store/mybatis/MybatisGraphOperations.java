...
public class MybatisGraphOperations implements GraphOperations {
    ...
    public void applyGraphWritePlan(
        ...
        refreshEntityCooccurrencesBestEffort(memoryId, normalizedPlan.affectedEntityKeys());
    }
    ...
    private void refreshEntityCooccurrencesBestEffort(
            MemoryId memoryId, Collection<String> affectedEntityKeys) {
        try {
            rebuildEntityCooccurrences(memoryId, affectedEntityKeys);
        } catch (RuntimeException error) {
            // Keep the session write durable even if a derived graph refresh becomes too slow.
            // The derived co-occurrence data can be rebuilt later.
        }
    }
