...
public class InMemoryRawDataOperations implements RawDataOperations {
    ...
    private static int boundaryStart(MemoryRawData rawData) {
        ...
        var boundary = rawData.segment().boundary();
        if (boundary
                instanceof
                com.openmemind.ai.memory.core.extraction.rawdata.segment.MessageBoundary mb) {
            return mb.startMessage();
        }
        if (boundary
                instanceof
                com.openmemind.ai.memory.core.extraction.rawdata.segment.CharBoundary cb) {
            return cb.startChar();
        }
