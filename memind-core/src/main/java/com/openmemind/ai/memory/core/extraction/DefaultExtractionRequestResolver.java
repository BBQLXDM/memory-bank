...
final class DefaultExtractionRequestResolver implements ExtractionRequestResolver {
    ...
    public Mono<ResolvedExtractionRequest> resolve(ExtractionRequest request) {
        ...
        if (source instanceof DirectContentSource directSource) {
            return resolveDirectContentRequest(request, directSource);
        }
        if (source instanceof FileExtractionSource fileSource) {
            return resolveFileExtractionRequest(request, fileSource);
        }
        if (source instanceof UrlExtractionSource urlSource) {
            return resolveUrlExtractionRequest(request, urlSource);
        }
        throw new IllegalArgumentException("Unsupported extraction source type: " + source);
    }
