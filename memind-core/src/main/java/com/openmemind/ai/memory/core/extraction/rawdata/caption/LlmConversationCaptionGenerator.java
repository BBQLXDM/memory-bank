...
public class LlmConversationCaptionGenerator implements CaptionGenerator {
    ...
    public LlmConversationCaptionGenerator(
    ...
    public Mono<String> generate(String content, Map<String, Object> metadata, String language) {
        ...
        var result = CaptionPrompts.build(promptRegistry, content, metadata).render(language);
        logPromptDiagnostics(
                "caption", language, content, result.systemPrompt(), result.userPrompt());
        ...
        return structuredChatClient
                ...
                .doOnNext(response -> logResponseDiagnostics("caption", response))
    ...
    private void logPromptDiagnostics(
            ...
            String userPrompt) {
        log.info(
                "Caption prompt diagnostics [{}]: language={} sourceHasChinese={} sourceHasAscii={}"
                    + " systemHasChinese={} systemHasAscii={} userHasChinese={} userHasAscii={}",
                stage,
                language,
                containsChinese(sourceText),
                containsAsciiLetters(sourceText),
                containsChinese(systemPrompt),
                containsAsciiLetters(systemPrompt),
                containsChinese(userPrompt),
                containsAsciiLetters(userPrompt));
        log.debug("Caption prompt [{}] system:\n{}", stage, systemPrompt);
        log.debug("Caption prompt [{}] user:\n{}", stage, userPrompt);
    }

    private void logResponseDiagnostics(String stage, CaptionResponse response) {
        if (response == null) {
            log.info("Caption response diagnostics [{}]: null", stage);
            return;
        }
        String text = response.toString();
        log.info(
                "Caption response diagnostics [{}]: hasChinese={} hasAscii={}",
                stage,
                containsChinese(text),
                containsAsciiLetters(text));
        log.debug("Caption response [{}]: {}", stage, text);
    }
