...
public final class ThreadDecisionEngine {
    ...
    public ThreadDecisionOutcome resolve(
        ...
        Double topCandidateScore = candidates.isEmpty() ? null : candidates.get(0).finalScore();
        ...
        if (exactAnchorMatches.size() == 1) {
            ThreadCandidateScore exactAnchorMatch = exactAnchorMatches.get(0);
        ...
        if (!candidates.isEmpty()) {
            ThreadCandidateScore topCandidate = candidates.get(0);
        ...
        CanonicalizedSignal creationSignal = signals.get(0);
    ...
    private static ThreadIntakeSignal representativeSignal(
        ...
        return canonicalSignals.stream()
                ...
                .findFirst()
                .orElseGet(() -> canonicalSignals.get(0).signal());
    }
}
