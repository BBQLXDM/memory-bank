...
public class InsightGroupRouter {
    ...
    public Mono<Map<String, List<MemoryItem>>> group(
            ...
            List<String> existingGroupNames,
            String language) {
        var strategy = category.groupingStrategy();
        if (strategy instanceof GroupingStrategy.LlmClassify) {
            return llmClassifier.classify(insightType, items, existingGroupNames, language);
        }
        if (strategy instanceof GroupingStrategy.MetadataField metadataField) {
            return Mono.just(groupByMetadata(items, metadataField.fieldName()));
        }
        throw new IllegalArgumentException("Unsupported grouping strategy: " + strategy);
    }
