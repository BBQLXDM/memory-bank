...
class PromptPreviewTest {
    ...
    @Test
    @DisplayName("preview should emit source-language rule in SOURCE mode")
    void previewSourceModeEmitsSourceLanguageRule() {
        String preview =
                PromptPreview.preview(
                        PromptRegistry.EMPTY, PromptType.MEMORY_ITEM_UNIFIED, "SOURCE");

        assertThat(preview).contains("# 绝对要求：输出语言 = 源文本语言");
        assertThat(preview).doesNotContain("Output Language = English");
    }

    @Test
    @DisplayName("previewExpanded should emit source-language rule in SOURCE mode")
    void previewExpandedSourceModeEmitsSourceLanguageRule() {
        String preview =
                PromptPreview.previewExpanded(
                        PromptRegistry.EMPTY, PromptType.MEMORY_ITEM_UNIFIED, "SOURCE");
