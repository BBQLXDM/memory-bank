...
class ConversationChunkerTest {
    ...
    class BasicChunkingTests {
        ...
        void shouldChunkByMessageCount() {
            ...
            assertThat(segments.get(0).boundary()).isEqualTo(new MessageBoundary(0, 4));
            assertThat(segments.get(0).content()).contains("user: 1");
            assertThat(segments.get(0).content()).contains("assistant: 4");

            // Second chunk: messages 4-7
            assertThat(segments.get(segments.size() - 1).boundary())
        ...
        void shouldReturnSingleChunkWhenLessThanConfig() {
            ...
            assertThat(segments.get(0).boundary()).isEqualTo(new MessageBoundary(0, 2));
        }
    ...
    class MessageFormattingTests {
        ...
        void shouldFormatMessagesCorrectly() {
            ...
            assertThat(segments.get(0).content()).isEqualTo(expectedContent);
        }
    }
    ...
    class RuntimeContextTests {
        ...
        void shouldExposeRuntimeContextInsteadOfPersistingSourceMessages() {
            ...
            assertThat(segments.get(0).metadata()).doesNotContainKey("messages");
            assertThat(segments.get(0).runtimeContext()).isNotNull();
            assertThat(segments.get(0).runtimeContext().startTime())
                    .isEqualTo(Instant.parse("2026-03-27T02:17:00Z"));
            assertThat(segments.get(0).runtimeContext().observedAt())
                    .isEqualTo(Instant.parse("2026-03-27T02:18:00Z"));
            assertThat(segments.get(0).runtimeContext().userName()).isEqualTo("Alice");
        }
    }
}
