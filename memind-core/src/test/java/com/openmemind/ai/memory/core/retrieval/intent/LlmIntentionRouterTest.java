...
@DisplayName("LlmIntentionRouter Unit Test")
class LlmIntentionRouterTest {
    ...
    void constructorWithPromptRegistryUsesOverrideInstruction() {
        ...
        var registry =
                InMemoryPromptRegistry.builder()
                        ...
                        .build();
        var router = new LlmIntentionRouter(client, registry);

        StepVerifier.create(
                        router.route(
                                DefaultMemoryId.of("u1", null),
                                "What did we decide?",
                                List.of("We discussed the rollout plan yesterday.")))
                .expectNext(RetrievalIntent.RETRIEVE)
                .verifyComplete();

        assertThat(client.lastMessages().get(0).content())
                .contains("Custom intent routing instruction");
    }
    ...
    void legacyConstructorDelegatesToEmptyPromptRegistry() {
        var client = new CapturingStructuredChatClient(new RuntimeException("LLM unavailable"));
        var router = new LlmIntentionRouter(client);
        ...
        assertThat(client.lastMessages().get(0).content()).contains("When to RETRIEVE");
    }
