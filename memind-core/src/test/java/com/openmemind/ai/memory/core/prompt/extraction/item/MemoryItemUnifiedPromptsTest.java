...
class MemoryItemUnifiedPromptsTest {
    ...
    @Test
    @DisplayName("Rendered prompt should preserve source language")
    void shouldPreserveSourceLanguage() {
    ...
    void shouldKeepKeyCommonConfusionExamples() {
        ...
        assertThat(systemPrompt)
                .contains("我们用 Redis/Kafka/X 做 Y")
                .contains("当前在学习/阅读/迁移 X")
                .contains("某同事负责后端服务")
                .contains("助手说‘温柔一点’")
                .contains("不要抽取，除非用户后来把它变成自己的长期实践或指令")
                .contains("用户有固定 SOP");
    }

    @Test
    @DisplayName("Rendered prompt should include correction and state change rules")
    void shouldIncludeCorrectionRules() {
        var systemPrompt = renderSystemPrompt(Set.of(MemoryCategory.values()));

        assertThat(systemPrompt)
                .contains("# Corrections & State Changes")
                .contains("必须额外生成一条「变更记录」记忆项")
                .contains("由「<旧值>」变更为「<新值>」")
                .contains("绝不要只保留最新状态而丢弃旧值");
        assertThat(systemPrompt).contains("2026年2月25日的拜访方式发生更正：由「电话沟通」变更为「现场拜访」");
    }
        ...
        assertThat(template.describeStructure())
                .contains(
                        "Sections: objective, principles, extractionScope, extractionBias,"
                                + " contentPreservation, correctionRules, outputLanguage,"
        ...
        assertThat(result.systemPrompt())
                .contains(
                        "Profile, behavior, directive, playbook, resolution, and tool items should"
                                + " normally set")
                .contains("only when the text itself states or clearly implies that time")
                .contains(
